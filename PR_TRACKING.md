...
4. Plan longitudinal ajouté au script Blender généré. Rotation empruntée par défaut à "left" (pas vérifiée contre un vrai `.blend` de référence comme les 6 rotations de vues l'ont été). **Confirmée visuellement le 2026-07-14 par Mathias sur un vrai modèle de vaisseau spatial** : la coupe longitudinale est bien centrée à Y=0 et parfaitement alignée avec les sections S1 à S8 dans Blender. Le commentaire "UNCONFIRMED" dans `blender_export.py` peut être retiré/adouci si on retouche ce fichier, mais ce n'est pas fait à ce jour.
...
**Bug de décalage trouvé et corrigé le 2026-07-14 (après coup, suite à un test utilisateur réel)** : les panneaux de coupe (côtes + longitudinale) exportés individuellement se centraient sur leur propre contenu plutôt que sur le vrai centre du maillage, contrairement aux vues qui elles utilisaient déjà `rib_y_center` pour l'axe vertical. Ça ne posait problème que sur l'axe horizontal (jamais corrigé pour cet axe avant). Concrètement : sur un modèle avec une géométrie asymétrique par rapport au plan de coupe, l'image de la vue (silhouette complète) et l'image du panneau de coupe (plan unique) ne s'alignaient pas quand on les superposait (~40cm d'écart mesuré sur un mesh de test synthétique). Corrigé en ajoutant `rib_x_center`/`long_x_center`, calculés en miroir de `rib_y_center`. Au passage, les traits pointillés + labels S1/S2 manquants sur `section_N.png`/`longitudinal.png` (jamais câblés lors du "prochain chantier" ci-dessus) ont aussi été ajoutés. Le tout confirmé par Mathias en conditions réelles (superposition Photoshop + script Blender).

**Corrections du 2026-07-16** (suite à code review de `tauri-launcher` + `split-view-export`) :
- **Formule des marqueurs dans `export_split_views()`** : le dénominateur `max_dim` (taille globale du canvas) a été remplacé par `padded_span` (content_span * 1.1, soit le même calcul que `compose_image()`). Conséquence : les positions des traits S1-SN sont maintenant cohérentes entre l'image composite et les PNG individuels exportés.
- **`label_sections` respecté dans `export_split_views()`** : la dette technique notée ci-dessous est soldée. Les numéros S1-SN sur les exports individuels (vues + panneau longitudinal) respectent maintenant la préférence, comme `compose_image()` le fait depuis l'origine. Corrigé sur `split-view-export`, propagé sur `blender-reference-script` et `tauri-launcher`.
